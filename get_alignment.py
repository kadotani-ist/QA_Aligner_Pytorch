...
def convert_to_alignment_format(s_toks, t_toks, align_matrix):
    align_list = []
    for i, s_tok in enumerate(s_toks):
        for j, t_tok in enumerate(t_toks):
            if align_matrix[i, j] > 0:
                align_list.append('{0}-{1}-{2:.4f}'.format(i, j, align_matrix[i, j]))
    return ' '.join(align_list)

def output_alignments(all_s_sents, all_t_sents, all_golds, all_predictions):
    with open(args.out, 'w') as fw:
        fw.write('Sent1\tSent2\tGold\tPred\n')
        for s_sent, t_sent, gold, pred in zip(all_s_sents, all_t_sents, all_golds, all_predictions):
            fw.write('{0}\t{1}\t{2}\t{3}\n'.format(s_sent, t_sent, gold, pred))
...
def main(args):
    ...
    all_s_sents, all_t_sents, all_golds, all_predictions = [], [], [], []
    with alignment_file_Path.open() as f:
        ...
        for line in f:
            ...
            if args.source_to_target:
                all_s_sents.append(f_line)
                all_t_sents.append(e_line)
                all_golds.append(a_line)
                # all_predictions.append(convert_to_alignment_format(f_toks, e_toks, f_to_e_align))
                all_predictions.append(convert_to_alignment_format(f_toks, e_toks, f_to_e_prob))
            elif args.target_to_source:
                all_s_sents.append(f_line)
                all_t_sents.append(e_line)
                all_golds.append(a_line)
                # all_predictions.append(convert_to_alignment_format(f_toks, e_toks, e_to_f_align.transpose()))
                all_predictions.append(convert_to_alignment_format(f_toks, e_toks, e_to_f_prob.transpose()))
            elif args.bidi_threshold:
                all_s_sents.append(f_line)
                all_t_sents.append(e_line)
                all_golds.append(a_line)
                # all_predictions.append(convert_to_alignment_format(f_toks, e_toks, bidi_align_th))
                all_predictions.append(convert_to_alignment_format(f_toks, e_toks, np.where(bidi_align_ave > 0.4, bidi_align_ave, 0)))
    ...
    for key in total_sys.keys():
        ...
        print('{}: {:.3f} {:.3f} {:.3f}'.format(key, recall, precision, f1), end=' ')
        print('({} {} {})'.format(total_ref, total_sys[key], total_common[key]))

    output_alignments(all_s_sents, all_t_sents, all_golds, all_predictions)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--nbest_predictions', '-n',
                        help='nbest predictions json file (char index)', required=True)
    parser.add_argument('--alignments', '-a', required=True)
